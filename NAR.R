...
for(n in n1:n2){
  ...
  #檢查原始logrt與配適結果
  #==========================================================================================================
  {plot(logrt[n:(n+249),2],type = 'l',col=4,lwd=7,xlab='',xaxt="n",ylab='logrt',main=paste0(name[c],'  ',ymd(logrt[(n+249),1])))
    points(as.numeric(fitted[n,2:251])+as.numeric(res[n,2:251])*as.numeric(sigma[n,2:251]),type='l',col=2,lwd=3)
    #points(as.numeric(fitted[n,2:251]),type='l',col=4,lwd=2)
    legend("bottomleft",legend=c(paste0(name[c]," Real logrt"),paste0(name[c]," check")),
           col=c(4,2), lty=1:1, cex=1.2, box.lty=0,bty="n")}
  #==========================================================================================================
  ...
  Q = para$Q[n]
  if(p>1){
    NARnotinmodel=NAR[(1:p),]             # p代表AR(p) 所以前p天不會有估計值，標準化殘差為0的前幾天不進入估計模型
    NARnotinmodel[,7]=NARnotinmodel[,6]   # 將不進入估計模型那幾天的fitted_residuals直接給定為residuals，而residuals為0
    NAR=NAR[-(1:p),]
    NAR=as.data.frame(NAR)
  }
  if(p==1){
    NARnotinmodel=as.matrix(t(NAR[(1:p),]))   # p代表AR(p) 所以前p天不會有估計值，標準化殘差為0的前幾天不進入估計模型
    NARnotinmodel[,7]=NARnotinmodel[,6]       # 將不進入估計模型那幾天的fitted_residuals直接給定為residuals，而residuals為0
    NAR=NAR[-(1:p),]
    NAR=as.data.frame(NAR)
  }
  ...
  # png(paste0(name[c],'  ',ymd(logrt[(n+249),1]),' 過去250天配適情況.png'), height = 800 , width = 1700)
  # {plot(rp[,1],type = 'l',xlab='',xaxt="n",ylab='logrt',main=paste0(name[c],'  ',ymd(logrt[(n+249),1]),' 過去250天配適情況'))
  #   points(rp[,3],type='l',col=4,lwd=2)
  #   points(rp[,5],type='l',col=2,lwd=2)
  #   abline(h=0,col=1)
  #   legend("bottomleft",legend=c(paste0(name[c]," Real logrt"), paste0(name[c]," ARMA-GARCH"),paste0(name[c]," NAR")),
  #          col=c(1,4,2), lty=1:1, cex=1.2, box.lty=0,bty="n")}
  # dev.off()
  #==========================================================================================================
  
  #預測結果
  #==========================================================================================================
  cat(x,name[c],' ',as.character(logrt[(n+249),1]),'\n')
  #cat('arma(',p,',',q,') garch(',P,',',Q,')\n')
  
  # 再配適一次arma garch模型
  model=eval(parse(text=paste0("garchFit(~arma(",p,",",q,")+garch(",P,",",Q,"),data=as.numeric(logrt[",n,":",(n+249),",2]),cond.dist = c('norm'),trace=F)")))
  at = as.numeric(sigma[n,2:251])*as.numeric(res[n,2:251])  # at為殘差項，計算第251天的預測值跟異質標準差會用到
  forecast[x,1] = as.numeric(logrt[(n+249+1),2])            # 預測的日期是配適後的下一天
  
  #####################################
  # 計算第251天的配適值(fitted value) #
  #####################################
  dd=251
  modelcoef=model@fit$matcoef[,1]  # 擷取 ARMA-GARCH 模型所有模型係數
  coef     =modelcoef[1:(1+p+q)]   # 擷取預測值(fitted value)所需要的係數
  r_hat=coef[1]
  j=1
  for(i in 2:(1+p)){r_hat = r_hat + (as.numeric(logrt[n:(n+249),2])[dd-j])*coef[i]; j=j+1}  # AR部分
  j=1
  if(length(coef) >= (1+p+1)){
    for(i in (1+p+1):length(coef)){r_hat = r_hat + (at[dd-j])*coef[i]; j=j+1}               # MA部分
  }
  # r_hat = as.numeric(r_hat)
  # as.numeric(fitted[n,2:251])[dd]
  # r_hat
  # 第三欄放ARMA-GARCH
  forecast[x,3] = as.numeric(r_hat)
  
  ###########################
  # 計算第251天的異質標準差 #
  ###########################
  coef = modelcoef[(1+p+q+1):length(modelcoef)]   # 擷取異質標準差(sigma)所需要的係數
  sigmat_2=coef[1]
  j=1
  for(i in 2:(1+P)){sigmat_2 = sigmat_2 + ((at[dd-j])^2)*coef[i]; j=j+1}                                       # at^2部分
  j=1
  for(i in (1+P+1):length(coef)){sigmat_2 = sigmat_2 + ((as.numeric(sigma[n,2:251])[dd-j])^2)*coef[i]; j=j+1}  # sigma部分
  # sigmat=as.numeric(sqrt(sigmat_2))
  # as.numeric(sigma[n,2:251])[dd]
  # sigmat
  sigma_pre[x,] = as.numeric(sqrt(sigmat_2))
  
  ############################
  # 擷取網絡效應項的模型參數 #
  ############################
  res.fit_coef = as.matrix(res.fit$coefficients)
  res.fit_coef[is.na(res.fit_coef)]=0             # 網絡效應項係數有可能是NA，要改成0
  
  setwd(paste0(dir_path,'/欣翰/market_2020')) ; getwd()
  full_res_pre=list()
  full_res_pre[[c]]=res_file1[,c(1,(c+1))]
  for(i in f[-c]){
    #cat(i,' ',name[i],'\n')
    temp=fread(paste0('residuals_',name[i],'.csv'),header=TRUE)  # 讀取要合併的國家的標準化殘差
    m=NULL
    if(adj_order[c,i]==0){b=0}else{b=1}  # 給定所有國家跟要預測的國家要差幾天，b=0代表差0天，b=1代表差1天
    while(length(m)==0){                 # 先找要合併的國家最新收盤日是哪一天
      #cat(b,'\n')
      m=which(as.Date(as.matrix(temp[,1]))==(as.Date(logrt[(n+249+1),1])+1-b))
      b=b+1
    }
    ...
    temp_1[,1]=as.matrix(temp[(m-249):m,1])        # 第一欄是要合併的國家的日期
    temp_1[,2]=as.numeric(temp[m,2:251])           # 第二欄是要合併的國家的標準化殘差
    full_res_pre[[i]]=temp_1
  }
  
  resforpre1=matrix(NA,1,nn)
  resforpre1[,c]=as.numeric(res[n,251])            # 第二欄是要預測的國家的最後一天的標準化殘差
  resforpre2=resforpre1
  resforpre3=resforpre1
  resforpre4=resforpre1
  resforpre5=resforpre1
  for(i in f[-c]){
    j=250
    #cat(name[i],'\n')
    temp_1=full_res_pre[[i]]  # 讀取要合併的國家的標準化殘差
    #============================================
    #cat('一期\n')
    m=NULL
    if(adj_order[c,i]==0){b=0}else{b=1}   # 給定所有國家跟要預測的國家要差幾天，b=-1代表差0天，b=0代表差1天
    while(length(m)==0){   # 找欲預測國家的日期對應要合併的國家的最新收盤日期是哪一天
      m=which(as.Date(temp_1[,1])==(as.Date(res_file1[j,1])+1-b))
      b=b+1 
    }
    resforpre1[1,i]=temp_1[m,2]
    #============================================
    #cat('二期\n')
    m=NULL
    if(adj_order[c,i]==0){b=0}else{b=1}  # 給定所有國家跟要預測的國家要差幾天，b=-1代表差0天，b=0代表差1天
    while(length(m)==0){   # 找欲預測國家的日期對應要合併的國家的最新收盤日期是哪一天
      m=which(as.Date(temp_1[,1])==(as.Date(res_file1[j,1])+1-1-b))
      b=b+1 
    }
    resforpre2[1,i]=temp_1[m,2]
    #============================================
    #cat('三期\n')
    m=NULL
    if(adj_order[c,i]==0){b=0}else{b=1}  # 給定所有國家跟要預測的國家要差幾天，b=-1代表差0天，b=0代表差1天
    while(length(m)==0){   # 找欲預測國家的日期對應要合併的國家的最新收盤日期是哪一天
      m=which(as.Date(temp_1[,1])==(as.Date(res_file1[j,1])+1-2-b))
      b=b+1 
    }
    resforpre3[1,i]=temp_1[m,2]
    #============================================
    #cat('四期\n')
    m=NULL
    if(adj_order[c,i]==0){b=0}else{b=1}  # 給定所有國家跟要預測的國家要差幾天，b=-1代表差0天，b=0代表差1天
    while(length(m)==0){   # 找欲預測國家的日期對應要合併的國家的最新收盤日期是哪一天
      m=which(as.Date(temp_1[,1])==(as.Date(res_file1[j,1])+1-3-b))
      b=b+1 
    }
    resforpre4[1,i]=temp_1[m,2]
    #============================================
    #cat('五期\n')
    m=NULL
    if(adj_order[c,i]==0){b=0}else{b=1}  # 給定所有國家跟要預測的國家要差幾天，b=-1代表差0天，b=0代表差1天
    while(length(m)==0){   # 找欲預測國家的日期對應要合併的國家的最新收盤日期是哪一天
      m=which(as.Date(temp_1[,1])==(as.Date(res_file1[j,1])+1-4-b))
      b=b+1 
    }
    resforpre5[1,i]=temp_1[m,2]
  }
  
  # 計算五個網絡效應項及預測值
  #==========================================================================================================
  NARpre = matrix(NA,1,6)
  #colnames(NARpre)=c('neteffect1','neteffect2','neteffect3','neteffect4','neteffect5','fitted_residuals')
  NARpre[,1] = adjacency1%*%as.numeric(resforpre1)
  NARpre[,2] = adjacency2%*%as.numeric(resforpre2)
  NARpre[,3] = adjacency3%*%as.numeric(resforpre3)
  NARpre[,4] = adjacency4%*%as.numeric(resforpre4)
  NARpre[,5] = adjacency5%*%as.numeric(resforpre5)
  NARpre[,6] = as.numeric(NARpre[1:5])%*%res.fit_coef       # 將五期網絡項和標準化殘差配適參數內積
  forecast[x,4] = forecast[x,3] + NARpre[,6]*sigma_pre[x,]  # 將'GARCH預測值'加上'預測的準化殘差乘'以'預測的異質標準差'
  #==========================================================================================================
  ...
  # ar1預測 #
  ###########
  forecast[x,2] = linear$coefficients[1] + linear$coefficients[2]*as.numeric(logrt[(n+249),2])  # linear$coefficients[1]是ar(1)的截距項
  #==========================================================================================================
  x=x+1
}
end=Sys.time()
...
# 儲存'預測值'
rownames(forecast)=as.matrix(logrt[(n1+249+1):(n2+249+1),1])  # 第一欄是要合併的國家的日期
setwd(paste0(dir_path,'/欣翰/預測結果(大)')) ; getwd()
write.csv(forecast,paste0(name[c],'預測值.csv'))
